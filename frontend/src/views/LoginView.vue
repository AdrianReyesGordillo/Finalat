  ...
  <div class="min-h-screen flex items-center justify-center bg-surface-50 px-4 login-force-light">
    <div class="w-full max-w-md">
      ...
      <div class="text-center mb-8">
        ...
        <h1 class="text-2xl font-bold text-primary-700">Bienvenido de vuelta</h1>
        <p class="text-primary-400 mt-1">Inicia sesión para acceder a tu asesor financiero</p>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-surface-200 p-6 sm:p-8">
        ...
        <form @submit.prevent="handleEmailLogin" class="space-y-4">
          <div>
            <label for="email" class="block text-sm font-medium text-primary-600 mb-1">Correo electrónico</label>
            <input
              id="email"
              v-model="email"
              type="email"
          ...
          <!-- Error message -->
          <div v-if="authStore.error" class="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-600">
            {{ authStore.error }}
          </div>
        ...
        <p class="text-center text-sm text-primary-400 mt-6">
          ¿No tienes cuenta?
          <RouterLink to="/registro" class="text-accent-500 hover:text-accent-600 font-semibold">
            Crear cuenta
          </RouterLink>
        </p>
      </div>
